# Base de dados da Bitoll - Setup

1. Create a base de dados da Bitoll project.
2. Fill `.env` with:
   - `PUBLIC_SUPABASE_URL`
   - `PUBLIC_SITE_URL` with the deployed app domain, for example
     `https://YOUR_DOMAIN`
   - `PUBLIC_SUPABASE_ANON_KEY` or `PUBLIC_SUPABASE_PUBLISHABLE_KEY`
3. In the base de dados da Bitoll, open SQL Editor and run `supabase/schema.sql`.
4. In the base de dados da Bitoll SQL Editor, run `supabase/seed.sql` to publish starter
...
7. In the base de dados da Bitoll Auth URL configuration, add the app URL:
   - Local: `http://localhost:5173/auth/callback`
   - Production: `https://bitoll.vercel.app/auth/callback`
   - Site URL: `https://bitoll.vercel.app`

The app uses the base de dados da Bitoll Auth for login and keeps the first local fallback until
